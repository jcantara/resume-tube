...
Meteor.methods({
  ...
  remove_playlist: function(playlist_id) {
    ...
    if(!!playlist && playlist.userId == this.userId) {
      ...
      throw new Meteor.Error('remove_playlist', 'Unauthorized playlist access');
    }
    return null;
  },
  create_playlist: function(url) {
    ...
    if (this.userId) {
      playlist_count = Playlists.find({ userId: this.userId }).fetch().length
      if(playlist_count >= 200) {
        throw new Meteor.Error('create_playlist', 'Too many playlists');
      }
      ...
      throw new Meteor.Error('create_playlist', 'Unauthorized playlist access');
    }
  }
});
